 ...
 * @returns {*}
 */
function getPrice(effect){
  var g = effect.got ? effect.got : effect.gets;
  var p = effect.paid ? effect.paid : effect.pays;

  //console.log(p.to_human_full());
  
  return p.to_number() == 0 ? 0 : g.ratio_human(p);
};
 ...
 */
   ...
   */
function filterAnodes(affectedNodes, type) {
 ...
 function processAnode(an) {
   ...
   */
 var getAmountSent = function (tx, meta) {
      ...
      meta.AffectedNodes.forEach(function (n) {
        ...
        // Offer
          ...
          if (effect.gets && effect.pays) {
        	  //console.log(tx.hash, effect.type);
